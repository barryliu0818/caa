...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        args.num_classes = 10
        mnist_val = torchvision.datasets.MNIST(root='.data/mnist', train=False,download=True, transform = transforms.ToTensor())
    ...
    if args.dataset == 'cifar10':
        ...
        args.num_classes = 10
        cifar10_val = torchvision.datasets.CIFAR10(root='.data', train=False,download=True, transform = transforms.ToTensor())
        ...
        if args.net_type == 'clean':
            ...
            from cifar_models.resnet import resnet50
            model = resnet50()
            model.load_state_dict({k[13:]:v for k,v in torch.load(('./checkpoints/cifar_linf_8.pt'),map_location='cuda')['state_dict'].items() if 'attacker' not in k and 'new' not in k})
            normalize = NormalizeByChannelMeanStd(
                mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])
            model = nn.Sequential(normalize, model)
            ...
            from cifar_models.resnet import resnet50
            model = resnet50()
            model.load_state_dict({k[13:]:v for k,v in torch.load(('./checkpoints/cifar_l2_0_5.pt'),map_location='cuda')['state_dict'].items() if 'attacker' not in k and 'new' not in k})
            normalize = NormalizeByChannelMeanStd(
                mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])
            model = nn.Sequential(normalize, model)
